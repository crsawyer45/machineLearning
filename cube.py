...
# [6 5 4]
    ...
    def getNumberOfCrossPiecesSolved(self):
        num = 0
        if self.back[5] == "B" and self.downer[5] == "W":
            num += 1
        if self.front[5] == "G" and self.downer[1] == "W":
            num += 1
        if self.left[5] == "R" and self.downer[3] == "W":
            num += 1
        if self.right[5] == "O" and self.downer[7] == "W":
            num += 1
        return num

    # randomly mixes the cube using 30 turns
    def mix(self):
        seq = []
        for i in range(30):
            random.seed(i)
